...
describe("app", () => {
  ...
  describe("GET /api/reviews/:review_id", () => {
    ...
    test("status 404 when searching not_a_review_title", () => {
      return request(app)
        ...
        .expect(404)
        .then(({ body }) => {
          expect(body.msg).toBe("This review does not exist or no such title!");
        });
    });
  ...
  describe("GET /api/reviews", () => {
    ...
    test("status 200 returns correct amount of reviews on certain page with category set sort by title and in ascending order by query", () => {
      return request(app)
        .get(
          "/api/reviews?limit=4&&p=3&&category=social%20deduction&&sort_by=title&&order=asc"
        )
        .expect(200)
        .then(({ body }) => {
          expect(body.reviews).toHaveLength(3);
  ...
  describe("POST /api/reviews", () => {
    ...
    test("status 404 failed to post due to invalid category in request body", () => {
      return request(app)
        ...
        .expect(404)
        .then(({ body }) => {
          expect(body.msg).toBe("No such category or username");
        });
    });
  ...
  describe("POST /api/users", () => {
    ...
    test("status 400 failed to post due to username already existed in database", () => {
      return request(app)
        ...
        .expect(400)
        .then(({ body }) => {
          expect(body.msg).toBe("This username already exist!");
        });
    });
