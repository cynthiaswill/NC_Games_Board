...
const uri = `mongodb+srv://yzhang4:${process.env.MCPASSWORD}@cluster0.rspyf.mongodb.net/My_test_project?retryWrites=true&w=majority`;
...
const getReviewSubsById = async (id) => {
  try {
    await client.connect();
    const database = client.db("My_test_project");
...
const subscribeReviewById = async (id, username) => {
  try {
    await client.connect();
...
const unsubscribeReviewById = async (id, username) => {
  try {
    await client.connect();
