...
exports.insertUser = async (username, name, url) => {
  ...
  const existedUsernames = usernames.rows.map((name) => name.username);
...
exports.getWatchedListByUsername = async (name) => {
  try {
    await client.connect();
    const database = client.db("My_test_project");
    const subscriptions = database.collection("users");
    const query = { username: `${name}` };
    const options = { projection: { watchedReviews: 1 } };
    const list = await subscriptions.findOne(query, options);
    if (list && list.watchedReviews) {
      console.log(list.watchedReviews, "watched_review_list fromo db");
      return list.watchedReviews;
    } else {
      return [];
    }
  } catch (error) {
